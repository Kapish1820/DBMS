...
def index():
    if request.method == 'POST':
        ...
        # Try-except to handle missing fields more cleanly
        try:
            ...
            if user and user['user_password'] == password_hashed:
                ...
                return redirect(url_for('portfolio'))  # Redirect to portfolio after successful login
            else:
                return render_template('alert2.html')  # Incorrect credentials

        except KeyError:
            return render_template('index.html', session=session)  # Missing form data
    else:
